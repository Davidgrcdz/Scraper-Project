...
public class BookingScrapper implements Scrapper{
    ...
    public void scrapper(int npaginas, String urlWithoutOffset) {
        ...
        for (int i = 0; i < npaginas; i++) {
            ...
            for (Element hotel : hotels) {
                try {
                    ...
                    BookingScrapper.hotels.add(hotelscraped);
    ...
    private Location locationscrapper(Document hotelHTML) {
        ...
        for (Element location: locations) hoteladress = location.text();
        return new Location(hoteladress, coordinates);
    }
